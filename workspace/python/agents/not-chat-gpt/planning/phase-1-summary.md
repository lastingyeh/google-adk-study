...
成功在 RAG 流程中整合了引用來源追蹤，確保 Agent 的回應不僅準確，而且有據可查，顯著降低了產生幻覺的風險。

### 核心實作流程

1. 工具回傳引用: 當 Agent 呼叫 file_search 工具後，Gemini File Search API 的回傳結果不僅包含找到的相關文本片段，還會自動附帶一個 citations 陣列，其中包含了每個文本片段的來源文件資訊。

2. ADK 自動中繼: Google ADK 框架會自動處理工具的輸出。它將 file_search 回傳的內容連同 citations 一起傳遞給 LLM。這一步是框架內建的，無需手動編碼。

3. LLM 參考生成: LLM 在生成最終答案時，會參考這些由工具提供的文本內容和來源資訊。這使得模型的回答是基於上傳的文檔內容，而非無根據的猜測。

4. 最終回應附加來源: 當 LLM 生成最終回應後，ADK 框架會將工具提供的 citations 資訊附加到最終的 API 回應中，供前端或客戶端使用。

### 驗證方法

- Trace 工具: 透過 ADK 的 trace 工具或在 Web UI 的 "Trace" 分頁中，可以清晰地看到整個呼叫鏈。可以驗證 file_search 工具的回應中是否包含 citations，以及這些資訊是否被正確地傳遞給了 LLM。
- API 回應: 檢查 /run 端點的最終 JSON 回應，確認其中包含了 citations 欄位及正確的來源文件資訊。

技術特色與優勢

- 降低幻覺: 這是此功能最核心的價值。透過強制模型基於提供的上下文作答，大大減少了模型捏造事實的可能性。
- 可追溯性: 使用者可以清楚地知道答案的來源是哪一份文件，增加了系統的可信度。
- 框架級支援: 整個引用追蹤流程大部分由 ADK 框架自動處理，開發者只需確保工具被正確呼叫，極大地簡化了實作複雜度。

完成狀態

- ✅ file_search 工具回傳 citations
- ✅ LLM 能參考引用來源生成回應
- ✅ 最終 API 回應包含引用資訊
- ✅ 透過 Trace 工具驗證流程正確性
