...
class ChatStreamUser(HttpUser):
    ...
    def chat_stream(self) -> None:
        ...
        步驟:
        1. 建立會話 (POST /apps/rag/users/{user_id}/sessions)
        2. 發送聊天請求 (POST /run_sse)
        3. 處理串流回應
        4. 檢查錯誤 (包含 429 Rate Limit 和 JSON 錯誤代碼)
        5. 觸發 Locust 事件記錄成功或失敗
        """
        ...
        session_response = self.client.post(
            f"/apps/rag/users/{user_id}/sessions",
            headers=headers,
            json=session_data,
        )
