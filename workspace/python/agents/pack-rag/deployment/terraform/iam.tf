...
# 3. 授予應用程式服務帳戶 (Application SA) 執行應用程式所需的權限
...
# 4. 允許 Cloud Run 服務帳戶拉取儲存在 CICD 專案中的容器
...
# 重點摘要
# - **核心概念**：IAM 權限管理
# - **關鍵技術**：IAM Member, Roles, Service Accounts
# - **重要結論**：集中管理跨專案的權限指派，包括 CI/CD Runner 的部署權限、應用程式執行時所需的資源存取權限，以及跨專案的 Artifact Registry 讀取權限。
# - **行動項目**：無
