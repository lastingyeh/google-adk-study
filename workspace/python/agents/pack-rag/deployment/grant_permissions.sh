...
# 授予 AI Platform Reasoning Engine 服務代理 RAG Corpus 存取權限的腳本
...
# 從 RAG_CORPUS 環境變數獲取 RAG Corpus ID
...
# 從完整的 RAG_CORPUS 路徑提取 RAG_CORPUS_ID
...
# 重點摘要
# - **核心概念**：權限管理 (IAM)
# - **關鍵技術**：gcloud CLI, IAM Custom Role, Service Identity
# - **重要結論**：此腳本為 AI Platform Reasoning Engine 的服務帳戶授予了查詢特定 RAG Corpus 的最小權限，確保安全性。
# - **行動項目**：在部署前執行此腳本以設定必要權限。
