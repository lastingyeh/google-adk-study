-- 優化 Cloud Logging 資料與 GCS 儲存的提示/回應資料的連接查詢。
-- 此查詢會提取日誌中參考的輸入和輸出訊息。
-- 注意：輸入檔案包含完整的對話歷史，因此訊息可能會出現多次。

-- 從 Cloud Logging 中提取訊息參考 (掃描一次，同時提取輸入/輸出)
WITH log_refs AS (
  SELECT
    insert_id,
    timestamp,
...
-- 依據 trace 去除重複：每個 trace 只保留最新的日誌條目
-- (工具呼叫會建立多個具有相同 trace 但不同時間戳記的日誌條目)
deduplicated AS (
  SELECT
    *,
    ROW_NUMBER() OVER (
