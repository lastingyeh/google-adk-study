...
此檔案包含對圖片載入、優化、範例圖片生成，以及多模態工具
（如 analyze_product_image, compare_product_images 等）的完整測試。
"""
...
class TestListSampleImages:
    ...
    async def test_list_sample_images_with_images(self, tmp_path):
        """測試當圖片存在時，列出範例圖片。"""
        # 如果 _sample_images/ 中存在真實圖片，此測試將通過
        mock_context = MagicMock()
    ...
    async def test_list_sample_images_structure(self):
        """測試 list_sample_images 回傳的結構是否正確。"""
        mock_context = MagicMock()
