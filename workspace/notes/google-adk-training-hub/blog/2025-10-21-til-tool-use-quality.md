...
```mermaid
graph TD
    subgraph Traditional["傳統評估 (僅限最終答案)"]
        Q1["代理回應"] --> Q2{"答案是否<br/>正確？"}
        Q2 -->|是| Q3["✅ 高分"]
        Q2 -->|否| Q4["❌ 低分"]
    end

    subgraph ToolUse["工具使用品質評估"]
        T1["工具序列"] --> T2{"順序是否<br/>正確？"}
        T2 -->|是| T3["✅ 高分"]
        T2 -->|否| T4["❌ 低分<br/>(即使答案正確)"]
    end
...
```mermaid
graph TB
    R1["<div style='text-align: left;'>🎯 工具選擇<br/>40% 權重</div>"] -->|得分 0.0-1.0| Calc["<div style='text-align: left;'>⚙️ 分數<br/>計算</div>"]
    R2["<div style='text-align: left;'>🔄 工具順序<br/>35% 權重</div>"] -->|得分 0.0-1.0| Calc
    R3["<div style='text-align: left;'>⚡ 組合效率<br/>15% 權重</div>"] -->|得分 0.0-1.0| Calc
    R4["<div style='text-align: left;'>🔧 錯誤恢復<br/>10% 權重</div>"] -->|得分 0.0-1.0| Calc

    Calc -->|加權平均| Final["<div style='text-align: left;'>📊 最終分數<br/>0.0 - 1.0</div>"]

    Final -->|≥ 0.8| Pass["<div style='text-align: left;'>✅ 優異<br/>工具品質</div>"]
    Final -->|0.6 - 0.8| Fair["<div style='text-align: left;'>⚠️ 良好<br/>有些許問題</div>"]
    Final -->|< 0.6| Fail["<div style='text-align: left;'>❌ 差勁<br/>重大問題</div>"]
...
```mermaid
graph TD
    Start["<div style='text-align: left;'>🚀 開始<br/>make evaluate</div>"] -->|建立測試案例| Step1["<div style='text-align: left;'>📝 步驟 1<br/>產生<br/>evalset.json</div>"]

    Step1 -->|3 個測試案例| Step2["<div style='text-align: left;'>⚙️ 步驟 2<br/>載入測試<br/>配置</div>"]

    Step2 -->|評分標準規則| Step3["<div style='text-align: left;'>🔍 步驟 3<br/>初始化<br/>AgentEvaluator</div>"]

    Step3 -->|Gemini 裁判| Step4["<div style='text-align: left;'>⚡ 步驟 4<br/>執行測試並<br/>比較序列</div>"]

    Step4 -->|工具調用| Step5["<div style='text-align: left;'>📊 步驟 5<br/>使用評分標準<br/>評分</div>"]

    Step5 -->|0.0-1.0 分數| Judge{"分數是否 ≥<br/>閾值？"}

    Judge -->|✅ 是| Result_Pass["<div style='text-align: left;'>✅ 通過<br/>良好的工具使用</div>"]
    Judge -->|❌ 否| Result_Fail["<div style='text-align: left;'>❌ 失敗<br/>需要改進</div>"]

    Result_Pass -->|報告| End["<div style='text-align: left;'>📈 結束<br/>查看結果</div>"]
    Result_Fail -->|報告| End
...
```mermaid
graph TD
    subgraph Good["✅ 良好的工具序列 (得分: 0.9)"]
        G1["<div style='text-align: left;'>1️⃣ get_customer<br/>客戶 ID</div>"]
        G2["<div style='text-align: left;'>2️⃣ get_orders<br/>客戶 ID</div>"]
        G3["<div style='text-align: left;'>3️⃣ calculate_refund<br/>訂單數據</div>"]
        G4["<div style='text-align: left;'>4️⃣ process_refund<br/>金額</div>"]

        G1 -->|結果| G2
        G2 -->|訂單| G3
        G3 -->|金額| G4
        G4 -->|退款已處理| G_Result["<div style='text-align: left;'>✅ 成功<br/>流程正確</div>"]
    end

    subgraph Bad["❌ 錯誤的工具序列 (得分: 0.35)"]
        B1["<div style='text-align: left;'>1️⃣ calculate_refund<br/>❌ 尚無數據！</div>"]
        B2["<div style='text-align: left;'>2️⃣ get_customer<br/>客戶 ID</div>"]
        B3["<div style='text-align: left;'>3️⃣ get_orders<br/>客戶 ID</div>"]
        B4["<div style='text-align: left;'>4️⃣ process_refund<br/>錯誤金額</div>"]

        B1 -->|錯誤| B2
        B2 -->|重試| B3
        B3 -->|訂單| B4
        B4 -->|退款失敗| B_Result["<div style='text-align: left;'>❌ 順序錯誤<br/>額外調用</div>"]
    end
