# 🛠 Deployment Handbook: Terraform & Helm Configuration

## 📖 摘要 (Abstract)
> 本手冊彙整了建置 GKE eBPF 安全平台所需的基礎程式碼配置（IaC）。內容包含使用 Terraform 部署支援 eBPF Dataplane V2 的 GKE 叢集，以及透過 Helm 安裝 Cilium 與 Tetragon 的詳細設定，並提供 Security Policy 範本供實作參考。
...
Tetragon 是基於 eBPF 的 Kubernetes-aware 安全觀測/執行工具，能追蹤進程執行、系統調用等行為。([Tetragon](https://tetragon.io/docs/getting-started/install-k8s/?utm_source=chatgpt.com))
...
這個策略 **在 Kernel 層攔截讀 `/etc/shadow` 的行為並 kill 該 process**。可延伸到更多事件（例如 execve, network connect 等）。([Is It Observable](https://isitobservable.io/observability/kubernetes/master-kubernetes-security-with-tetragon?utm_source=chatgpt.com))
...
✔ Helm 安裝 Cilium + Tetragon（並開啟 eBPF Runtime Security）([Tetragon](https://tetragon.io/docs/getting-started/install-k8s/?utm_source=chatgpt.com))
