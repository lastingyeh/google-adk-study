...
* eBPF 核心在 Kernel 內核層執行高效能路由、ACL、Policy 及流量觀察。([Google Cloud Documentation][1])

📌 **重點**

* 網路流量經過 eBPF，不需要 Sidecar 或 iptables，效能高且可觀測。
* 可收集 Layer3/4 的 Flow events（例如連線建立、拒絕等）。([Google Cloud Documentation][2])
...
* 捕捉 App 進程的 syscall、安全事件（ex: execve、file access、namespace changes）。([Medium][3])
* 可根據 Policy 決定是否執行阻斷行為（Runtime Enforcement）。([Tetragon][4])
...
* Cilium / GKE Dataplane V2 透過 eBPF maps 決定 flow path（更快 & context-aware）([Google Cloud Documentation][1])

📍 **Security Event Layer**

* Tetragon 監控系統呼叫、進程事件、Network & File 活動，提供策略觸發與阻斷能力([Tetragon][4])
...
✔ 優先啟用 **GKE Dataplane V2 observability** + **Hubble** 以捕捉網路 flow。([Google Cloud][5])
✔ 使用 **Tetragon Policy** 定義如 Syscall 探測、文件訪問策略。([Tetragon][4])
...
[1]: https://docs.cloud.google.com/kubernetes-engine/docs/concepts/dataplane-v2?utm_source=chatgpt.com "GKE Dataplane V2 | GKE networking"
[2]: https://docs.cloud.google.com/kubernetes-engine/docs/concepts/about-dpv2-observability?utm_source=chatgpt.com "About GKE Dataplane V2 observability | GKE networking"
[3]: https://medium.com/%40nonickedgr/cilium-tetragon-next-generation-runtime-security-for-kubernetes-41cfee727503?utm_source=chatgpt.com "Cilium Tetragon: Next-Generation Runtime Security for ..."
[4]: https://tetragon.io/?utm_source=chatgpt.com "Tetragon - eBPF-based Security Observability and Runtime ..."
[5]: https://cloud.google.com/blog/products/containers-kubernetes/using-hubble-for-gke-dataplane-v2-observability?utm_source=chatgpt.com "Using Hubble for GKE Dataplane V2 observability"
