...
它可以在核心層捕捉 Syscall、Network Packet、Process Event、File I/O 等行為，而不需要修改應用程式程式碼或容器映像。([Wikipedia][1])

---

## 2️⃣ **WHAT — 什麼是 eBPF Security 平台？**
...
這類能力是 Cilium / Tetragon 這些 eBPF 工具的核心特性。([Tetragon][2])
...
👉 這些都可以透過 eBPF 直接在核心層觀察與防止，而不是 Application 層。([Tetragon][2])

---

## 3️⃣ **WHO — 誰來用與誰來建？（角色與責任）**
...
* **Tetragon** 進一步提供運行時安全執行與策略 enforcement（可視情況選用）([Google Cloud][3])
...
## 6️⃣ **HOW — 如何從 0 到 1 實作（步驟與建議）**

### 🛠 Step 0 — 技術準備

**先學習 eBPF 與相關工具**

* eBPF 本質：在 Linux 核心中執行安全 sandboxed 程式，不需重編 Kernel。([Wikipedia][1])
* 了解 Cilium 和 Tetragon 在 Kubernetes 中是如何使用 eBPF 提供可觀察與安全功能的。([eBPF][4])
...
這樣 GKE 就會在 Node 上啟用 eBPF datapath，提升網路可觀察與效率。([Google Cloud][3])
...
Cilium 具備 Identity-Aware Policies、Flow Logs、透明加密等安全功能。([Wikipedia][5])
...
* 防止異常網路連線
* File integrity 事件
  這些都是 Tetragon 的典型能力。([Tetragon][2])
...
這些政策可以用 Tetragon、OPA、CRD 方式定義，eBPF 則實際在 Kernel 層監控與 enforce。([Tetragon][2])
...
能在 **Kernel 核心階段捕捉、分析、阻斷安全相關行為**。([Google Cloud][3])

--
## 參考資料
[1]: https://en.wikipedia.org/wiki/EBPF?utm_source=chatgpt.com "EBPF"
[2]: https://tetragon.io/?utm_source=chatgpt.com "Tetragon - eBPF-based Security Observability and Runtime ..."
[3]: https://cloud.google.com/blog/products/containers-kubernetes/bringing-ebpf-and-cilium-to-google-kubernetes-engine?utm_source=chatgpt.com "Bringing eBPF and Cilium to Google Kubernetes Engine"
[4]: https://ebpf.io/zh-hant/applications/?utm_source=chatgpt.com "eBPF 應用項目概覽"
[5]: https://en.wikipedia.org/wiki/Cilium_%28computing%29?utm_source=chatgpt.com "Cilium (computing)"
