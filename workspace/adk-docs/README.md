...
| 標頭                                                     | 描述                                                                                                         | 連結                           |
| :------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------- | :----------------------------- |
| [為什麼要評估代理 (Agents)](./evaluate/index.md)         | 說明代理評估的必要性、評估目標與成功標準，以及如何用測試檔/評估集 (EvalSet) 建立自動化評估流程。             | [連結](./evaluate/index.md)    |
| [評估標準 (Evaluation Criteria)](./evaluate/criteria.md) | 彙整 ADK 提供的評估準則（工具軌跡、最終回應品質、幻覺、Safety、量表 Rubrics、LLM-as-a-Judge 等）與使用時機。 | [連結](./evaluate/criteria.md) |
| [使用者模擬 (User Simulation)](./evaluate/user-sim.md)   | 介紹以 ConversationScenario 動態產生使用者回合進行對話式評估，以及相關 EvalConfig / user simulator 設定。    | [連結](./evaluate/user-sim.md) |
