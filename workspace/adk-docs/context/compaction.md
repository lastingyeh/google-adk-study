...
> 🔔 `更新日期：2026-01-30`
>
> 🔗 `資料來源`：https://google.github.io/adk-docs/context/compaction/
...
上下文壓縮功能使用「滑動視窗 (sliding window)」方法，在 [會話 (Session)](../sessions&memory/session/index.md) 內收集並摘要 Agent 工作流程事件資料。當您在 Agent 中配置此功能時，一旦工作流程事件或調用達到特定數量的閾值，它就會摘要來自較舊事件的資料。
