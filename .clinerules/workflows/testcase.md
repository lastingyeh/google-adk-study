...
## 執行步驟
從使用者提供的測試目錄 <dir>：
1. 在測試目錄確認 `README.md`是否存在，若不存在則建立。
1. 讀取每個測試檔案內容(例如 `test_*.py`)，並更新內容註解為繁體中文並加入註解重點說明。
2. 分析檔案內容，提取關鍵資訊（如測試目的、輸入資料、預期結果等）。
3. 根據產出範本`templates/testcase-template.md`整合格式化輸出，若欄位無法對應則顯示 `None`。
4. 根據提取的資訊，生成結構化的測試案例說明並更新相同目錄下檔案 `README.md`內容。

## 輸出檔案
更新所有 `tests`目錄下檔案內容儲存至`{file01}`目錄下的 `README.md`
